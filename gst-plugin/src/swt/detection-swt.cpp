...
#include "detection-swt.h"
#include "swt-wrapper.h"

#define PI 3.14159265

IplImage* unsigned_char_pgm_to_iplimage (unsigned char *image, int xmin, int ymin, int xmax, int ymax, int nrows, int ncols) {
   int nncols = (xmax - xmin);
   int nnrows = (ymax - ymin);
   IplImage *out = cvCreateImage (cvSize(nncols,nnrows), IPL_DEPTH_8U, 1); /*Grayscale*/
   for (int y = ymin; y < ymax; y++) {
      for (int x = xmin; x < xmax; x++) {
         out->imageData[(y - ymin) * nncols + (x - xmin)] = image[y * ncols + x];
      }
   }
   return out;
}
...
unsigned char* iplimage_to_unsigned_char_pgm (IplImage *image) {
   unsigned char *out = (unsigned char *)malloc(image->width*image->height*sizeof(unsigned char)); /*Grayscale*/
   IplImage *grayImage = cvCreateImage (cvGetSize (image), IPL_DEPTH_8U, 1 );
   cvCvtColor (image, grayImage, CV_RGB2GRAY);
   int ncols = grayImage->width; 
   int nrows = grayImage->height;
   int bpp = grayImage->nChannels;
   for (int i = 0; i < (ncols * nrows * bpp); i += bpp) {
      out[i] = (unsigned char)grayImage->imageData[i];
   }
   return out;
}
...
unsigned char* textDetection (IplImage * input, bool dark_on_light, double threshold_low, double threshold_high)
{
    ...
    unsigned char *out = iplimage_to_unsigned_char_pgm (output5);
...
void write_unsigned_char_to_pgm (char *filename, unsigned char *img,
		int nrows, int ncols)
{
...
static void free_vector_elemets2(Vector *v)
{
	while (v->count--) {
...
void copy_sub_image (unsigned char *src, int sncols, int snrows, unsigned char *dst, int dx, int dy, int dncols, int dnrows) {

   for (int y = 0; y < snrows; y++) {
      for (int x = 0; x < sncols; x++) {
         dst[(y + dy)*dncols + (x + dx)] = src[y * sncols + x]; 
      } 
   }
}

vector<plate> swt_text_filtering(unsigned char *img, int nrows, int ncols,
                ...
                double *prob, struct_thog sthog, int iframe)
{
        ...
        for (i = 0; i < nsets; i++) {
                ...
                if (classify (img, nrows, ncols, xmin, ymin, wbox, hbox, model, prob, sthog) > 0) {
...
vector<plate> no_filtering (unsigned char *img, int nrows, int ncols, Vector** chains, int nsets, int iframe)
{
        int i, j, k;
  ...
  //double threshold_low = 175;  /*Valores originais*/  /*Precision: 0.736264 */
  //double threshold_high = 320;  /*Recall: 0.587719*/
  //double threshold_low = 50;   /*Precision: 0.780702*/
  //double threshold_high = 100; /*Recall: 0.780702*/
  //double threshold_low = 30;   /*Precision: 0.823009 com filtragem por t-hog*/
  //double threshold_high = 100; /*Recall: 0.815789*/
  //double threshold_low = 20;   /*Precision: 0.810811*/
  //double threshold_high = 80; /*Recall: 0.789474*/
  //double threshold_low = 40;   /*Precision: 0.823009*/
  //double threshold_high = 100; /*Recall: 0.815789*/
...
vector<plate> detection_swt(
    ...
    int iframe)
{
    IplImage * byteQueryImage;
    vector<plate> plates;
    struct svm_model* model;
    double *prob;
    struct_thog *sthog;
    char out_image_name[32];
    char fname[256];
    ...
    /*Parameter settings: */
        /*Canny edge detector:*/
        double threshold_low = 175;            /*change*/
        double threshold_high = 320;           /*change*/
        /*Grouping filtering*/
        ...
        double t3 = 0.4;                       /*fixed*/
        /*T-HOG filtering: */
        bool filter = false;                   /*fixed*/
    /*End settings*/

        unsigned char *nswt = (unsigned char *)malloc(nrows * ncols * sizeof(unsigned char));
        memset (nswt, 0, nrows * ncols * sizeof(unsigned char)); 
        unsigned char *iswt = (unsigned char *)malloc(nrows * ncols * sizeof(unsigned char));
        memset (iswt, 0, nrows * ncols * sizeof(unsigned char)); 
        ...
        if (0 == cter) { return plates; }
  
        for (int i = 0; i < slopes.size(); i++) {
           if (!slopes.at(i).search_plate) { 
              continue;
           }
   	   IplImage *cv_aux = unsigned_char_pgm_to_iplimage (image, slopes.at(i).left, slopes.at(i).up, slopes.at(i).right, slopes.at(i).down, nrows, ncols);
           IplImage *cv_tmp = cvCreateImage (cvGetSize(cv_aux), IPL_DEPTH_8U, 3);
	   cvCvtColor(cv_aux, cv_tmp, CV_GRAY2RGB);
           unsigned char *nout = textDetection(cv_tmp, 1, threshold_low, threshold_high);
           unsigned char *iout = textDetection(cv_tmp, 0, threshold_low, threshold_high);
           copy_sub_image (iout, cv_tmp->width, cv_tmp->height, iswt, slopes.at(i).left, slopes.at(i).up, ncols, nrows);
           copy_sub_image (nout, cv_tmp->width, cv_tmp->height, nswt, slopes.at(i).left, slopes.at(i).up, ncols, nrows);
	   cvReleaseImage(&cv_aux);
	   cvReleaseImage(&cv_tmp);
           free(iout);
           free(nout);
        }
	//write_unsigned_char_to_pgm(fname, iswt, nrows, ncols);
	
	Vector ncc = find_connected_components4 (nswt, nrows, ncols, reg_rmin, reg_rmax);
	Vector icc = find_connected_components4 (iswt, nrows, ncols, reg_rmin, reg_rmax);

	int nsets;
        Vector **nchains = make_chains_and_merge (ncc, icc, t1, t2, t3, &nsets);

        if (filter) {
            plates = swt_text_filtering (image, nrows, ncols, nchains, nsets, s->model, s->prob, s->sthog, iframe);
        }
        else {
            plates = no_filtering (image, nrows, ncols, nchains, nsets, iframe);
        }

	chain_free(nchains, nsets); 
