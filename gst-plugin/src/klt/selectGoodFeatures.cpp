  ...
  KLT_BOOL overwriteAllFeatures)
{
  ...
  if (NULL == featuremap) {
	  KLTError("%s: could not alloc memory\n", __func__);
  }
  ...
  plate pl)
{
  ...
  if (mode == REPLACING_SOME &&
    ...
    if (tc->smoothBeforeSelecting)  {
      ...
      _KLTComputeSmoothedImageB(tmpimg, _KLTComputeSmoothSigma(tc), floatimg, pl);
      _KLTFreeFloatImage(tmpimg);
    } else _KLTToFloatImage(img, ncols, nrows, floatimg);

    /* Compute gradient of image in x and y direction */
    _KLTComputeGradientsB(floatimg, tc->grad_sigma, gradx, grady, pl);
  }
     ...
     of the two eigenvalues of the Z matrix */
  {
    ...
    assert((pl.h > 2 * safe_margin) && (pl.w > 2 * safe_margin));

      //for (k = 0; k < plates.size(); k++) {
      for (y = pl.y + safe_margin; y < pl.y + pl.h - safe_margin; y += tc->nSkippedPixels + 1)
         for (x = pl.x + safe_margin; x < pl.x + pl.w - safe_margin; x += tc->nSkippedPixels + 1)  {
           /* Sum the gradients in the surrounding window */
           gxx = 0;  gxy = 0;  gyy = 0;
           for (yy = y-window_hh ; yy <= y+window_hh ; yy++)
             for (xx = x-window_hw ; xx <= x+window_hw ; xx++)  {
                if ((ncols*yy+xx < 0) || (ncols*yy+xx > nrows*ncols)) { continue; }
  ...
  //featurelist->ref = _KLTCreateFloatImage (ncols, nrows);
  featurelist->xmin = pl.x;
  featurelist->ymin = pl.y;
  featurelist->xmax = pl.x + pl.w;
  featurelist->ymax = pl.y + pl.h;
  featurelist->dx = 0.0;
  featurelist->dy = 0.0;

  //_KLTToFloatImage (img, ncols, nrows, featurelist->ref);
...
{
	_KLTSelectGoodFeatures(tc, img, ncols, nrows, fl, SELECTING_ALL, pl);
}
		...
		int ncols, int nrows, KLT_FeatureList fl, plate pl)
{
	...
	if (nLostFeatures > 0) {
		_KLTSelectGoodFeatures(tc, img, ncols, nrows, fl,
				REPLACING_SOME, pl);
	}
}
