  ...
  char *line)
{
  ...
  while (line[0] == '\0')  {
    ...
    if (ret == 0)
      KLTError("(_getNextString) Bad retunr from fscanf");
    i = -1;
    do  {
      i++;
      if (line[i] == '#')  {
  ...
  int *maxval)
{
  ...
  *nrows = atoi(line);
  ...
  _getNextString(fp, line);
  *maxval = atoi(line);
  size = fread(line, 1, 1, fp); /* Read newline which follows maxval */
  if (size != 1)
    KLTError("(pnmReadHeader) Bad return os fread");
...
unsigned char* pgmRead(FILE *fp, unsigned char *img, int *ncols, int *nrows)
{
	...
	/* Read binary image data */
	{
		...
		for (i = 0; i < *nrows; i++) {
			size = fread(tmpptr, *ncols, 1, fp);
			if (size != 1) {
				KLTError("(pgmRead) Bad return from fread");
			}
			tmpptr += *ncols;
		}
	}
