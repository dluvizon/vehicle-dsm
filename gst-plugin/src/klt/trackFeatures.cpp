  ...
  _FloatWindow grady)      /*   " */
{
  ...
  //printf("Alpha: %f, mean1: %f, mean2: %f\n", alpha, mean1, mean2);
  ...
  int iframe)
{
  ...
  do  {
    ...
    if (!isnan(ex) && !isnan(ey) && !isnan(gxx) && !isnan(gxy) && !isnan(gyy) && !isnan(*x2) && !isnan(*y2)) {
 ...
 */
void KLTTrackFeatures(KLT_TrackingContext tc, int ncols, int nrows,
		KLT_FeatureList featurelist, int iframe, int iFeature)
{
	...
	/* Check window size (and correct it if necessary) */
	...
	if (tc->window_width < 3) {
		tc->window_width = 3;
		KLTWarning("Tracking context's window width must be at "
				"least three.\nChanging to %d.\n",
				tc->window_width);
	}
	if (tc->window_height < 3) {
		tc->window_height = 3;
		KLTWarning("Tracking context's window height must be at "
				"least three.\nChanging to %d.\n",
				tc->window_height);
	}
	...
	pyramid2 = (_KLT_Pyramid) tc->pyramid_act;
	pyramid2_gradx = (_KLT_Pyramid) tc->pyramid_act_gradx;
	pyramid2_grady = (_KLT_Pyramid) tc->pyramid_act_grady;

	if (pyramid1->ncols[0] != ncols || pyramid1->nrows[0] != nrows) {
		KLTError("(KLTTrackFeatures) Size of incoming image (%d by %d)"
				"is different from size of previous image "
				"(%d by %d)\n", ncols, nrows,
				pyramid1->ncols[0], pyramid1->nrows[0]);
	}
	...
	for (indx = 0 ; indx < featurelist->nFeatures ; indx++)  {
		...
		if (featurelist->feature[indx]->val >= 0)  {
                        featurelist->feature[indx]->prev_x =
				featurelist->feature[indx]->x; 
                        featurelist->feature[indx]->prev_y =
			...
			/* Transform location to coarsest resolution */
                        double dx = featurelist->dx;
                        double dy = featurelist->dy;
			...
			if (val == KLT_OOB) {
				...
				if (featurelist->feature[indx]->aff_img) {
					_KLTFreeFloatImage(featurelist->
							feature[indx]->aff_img);
				}
				if (featurelist->feature[indx]->aff_img_gradx) {
					_KLTFreeFloatImage(featurelist->
							feature[indx]->aff_img_gradx);
				}
				if (featurelist->feature[indx]->aff_img_grady) {
					_KLTFreeFloatImage(featurelist->
							feature[indx]->aff_img_grady);
				}
		...
		const KLT_PixelType *img, int ncols, int nrows)
{
	...
	/* Convert the input image, img, to a float image, stored in bufs->tmp. */
	_KLTToFloatImage(img, ncols, nrows, bufs->tmp);

	_KLTComputeSmoothedImageC(featurelist, bufs->tmp,
			_KLTComputeSmoothSigma(tc), bufs->aux, 0, tc);

	_KLT_Pyramid pyramid = _KLTCreatePyramid(ncols, nrows,
			tc->subsampling, tc->nPyramidLevels);

	_KLTComputePyramid(bufs->aux, pyramid, tc->pyramid_sigma_fact);

	_KLT_Pyramid pyramid_gradx = _KLTCreatePyramid(ncols, nrows,
			tc->subsampling, tc->nPyramidLevels);

	_KLT_Pyramid pyramid_grady = _KLTCreatePyramid(ncols, nrows,
			tc->subsampling, tc->nPyramidLevels);

	for (ind = 0 ; ind < tc->nPyramidLevels; ind++) {
		_KLTComputeGradientsC(featurelist, pyramid->img[ind],
				tc->grad_sigma, pyramid_gradx->img[ind],
				pyramid_grady->img[ind], ind, tc);
	}

	if (tc->pyramid_last != NULL) {
		/* Cleaning the previous image pyramid: */
		...
		/* Swapping the current image for the last pyramid: */
		tc->pyramid_last = tc->pyramid_act;
		tc->pyramid_last_gradx = tc->pyramid_act_gradx;
		tc->pyramid_last_grady = tc->pyramid_act_grady;
	} else {
		/* Just copy {pyramid} just created, as we do not have a
		 * previous one: */
		tc->pyramid_last = _KLTCopyPyramid(pyramid);
		tc->pyramid_last_gradx = _KLTCopyPyramid(pyramid_gradx);
		tc->pyramid_last_grady = _KLTCopyPyramid(pyramid_grady);
	}

	/* Updating the current pyramid pointer: */
	tc->pyramid_act = pyramid;
	tc->pyramid_act_gradx = pyramid_gradx;
	tc->pyramid_act_grady = pyramid_grady;
}
