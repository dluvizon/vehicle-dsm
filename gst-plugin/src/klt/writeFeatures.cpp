...
static const char warning_line[] = "!!! Warning: This is a KLT data file.  "
  ...
  char *filename)
{
  ...
  int k;
  float *ptrout = greyimg->data;
  for (k = 0; k < ncols*nrows; k++) {
     *(redimg + k) = (uchar)(*ptrout);
     *(grnimg + k) = (uchar)(*ptrout);
     *(bluimg + k) = (uchar)(*ptrout);
     ptrout++;
  }
...
#define LINELENGTH 100
  ...
  size = fread(line, sizeof(char), BINHEADERLENGTH, fp);
  if (size != BINHEADERLENGTH)
    KLTError("(_readHeader) Bad return from fread");

  line[BINHEADERLENGTH] = 0;
  if (strcmp(line, binheader_fl) == 0)  {
    assert(nFeatures != NULL);
    size = fread(nFeatures, sizeof(int), 1, fp);
    if (size != 1)
      KLTError("(_readHeader) Bad return from fread");
    *binary = TRUE;
    return FEATURE_LIST;
    ...
    assert(nFrames != NULL);
    size = fread(nFrames, sizeof(int), 1, fp);
    if (size != 1)
      KLTError("(_readHeader) Bad return from fread");
    *binary = TRUE;
  ...
  while (strcmp(line, warning_line) != 0)  {
    if (!fgets(line, LINELENGTH, fp))
      KLTError("(_readHeader) Bad return from fgets");
    if (feof(fp))
  ...
  ret = fscanf(fp, "%s", line);
  if ((strcmp(line, "=") != 0) && ret)
    KLTError("(_readFeatures) File is corrupted -- "
             "(Expected '=', found '%s' instead)", line);
  if (id == FEATURE_LIST) {
	  ret = fscanf(fp, "%d", nFeatures);
  }
  else {
  	ret = fscanf(fp, "%d", nFrames);
  }
  if (!ret)
    KLTError("(KLTReadFeatureTable) Bad return from scanf");
  ...
  if (id == FEATURE_TABLE)  {
    ...
    ret = fscanf(fp, "%s", line);
    if ((strcmp(line, "nFeatures") != 0) && ret)
      KLTError("(_readFeatures) File '%s' is corrupted -- "
  ...
  KLT_Feature feat)
{
  ...
  while (fgetc(fp) != '(');
  ret = fscanf(fp, "%f,%f)=%d", &(feat->x), &(feat->y), &(feat->val));
  if (!ret)
    KLTError("(KLTReadFeatureTable) Bad return from scanf");
}
  ...
  KLT_Feature feat)
{
  ...
  size = fread(&(feat->x), sizeof(KLT_locType), 1, fp);
  if (size != 1)
	  goto err;
  size = fread(&(feat->y), sizeof(KLT_locType), 1, fp);
  if (size != 1)
	  goto err;
  size = fread(&(feat->val), sizeof(int), 1, fp);
  if (size != 1)
	  goto err;

  return;
err:
  KLTError("(_readFeatureBin) Bad return from fread");
}
