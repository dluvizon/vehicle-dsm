...
//static const int mindist = 4;
//static const int window_size = 15;
static const int window_size = 11;
static const int min_eigenvalue = 1;
//static const int min_eigenvalue = 5;
...
//static const KLT_BOOL sequentialMode = TRUE;
//static const KLT_BOOL lighting_insensitive = FALSE;
static const KLT_BOOL lighting_insensitive = TRUE;
/* for affine mapping*/
...
KLT_TrackingContext KLTCreateTrackingContext()
{
  ...
  tc->pyramid_act = NULL;
  tc->pyramid_act_gradx = NULL;
  tc->pyramid_act_grady = NULL;
  ...
  int nFeatures)
{
  ...
  for (i = 0 ; i < nFeatures ; i++) {
    ...
    fl->feature[i]->val = 0;
    for (k = 0; k < SMPSIZE; k++) {
       fl->feature[i]->prv_x[k] = fl->feature[i]->prv_y[k] = NOT_FILLED;
       fl->feature[i]->ipm_x[k] = fl->feature[i]->ipm_y[k] = NOT_FILLED;
    }
  }
  /* Return feature list */
  return(fl);
